...
IPSec tunnels are used to securely connect private space subnets (rfc1918) over a negotiated connection between two routers or firewalls (commonly called peer endpoints with reference to IPSec). Generally, the two peer endpoints have network connectivity to each other via the internet, and if they don't have a direct connection over the internet, one or both peers may be behind NAT/port forwarding. In that latter case, NAT traversal must be enabled in the configurations. In the IBM Cloud® environment specifically, the Vyatta (or other classic gateway appliance) is used as the IBM Cloud® classic infrastructure side peer endpoint and the remote endpoint (with reference to IBM Cloud®) is a router or firewall on the "on-prem" side. Figure 1. below shows a very basic visual representation of an IPSec tunnel from IBM Cloud Classic to On-Prem over the internet.

![Vyatta basic IPSect tunnel configuration](images/VyattaIPSecBasic.jpeg "Vyatta basic IPSect tunnel configuration"){: caption="Figure 1: Vyatta basic IPSect tunnel configuration" caption-side="bottom"}
...
When applying firewall rulesets to your interfaces, be aware of what the source IP and destination IP address of a packet will be when going into and out of an interface. The following diagram illustrates a typical packet traversing the Vyatta on the forward and return path through an IPSec tunnel. The OUT direction for a vti is when the packet will be encrypted and sent out of the Vyatta. The IN direction for a vti is when the packet will be decrypted and sent to the server behind the Vyatta. For the outside interface, dp0bond1, the packet will always be the peer endpoint IP addresses and the port/protocols are ESP, UDP port 500 and UDP port 4500 (if NAT-T is used). Note that the sourece IP and destination IP in a packet for the IN direction for a VLAN interface (VIF) should match the OUT direction for the vti.
...
1. Reset tunnels and restart the VPN. Do this first if you can, as this can save time during an outage. Ensure you remember what each command is restarting and its effect on the rest of the tunnels.
1. Check for end-to-end network connectivity between peers. Also ensure that UDP ports 500 and 4500 (if using NAT-T), and protocol ESP are allowed on the outside interface for both peers. This is very important and widely overlooked.
1. Check phase 1 status. A "Down" status could mean:
   * Step 2 was not properly checked
   * Mismatching IKE versions, encryption algorythms, hash algorythms, or IKE Identifiers
   * Vendor incompatibilities
1. Check phase 2 status. A "Down" status (or no status at all) could mean: 
   * Phase 1 is down
   * Mismatching encryption algorythms, hash algorythms, traffic selectors, or dh-groups
   * Vendor incompatabilities
