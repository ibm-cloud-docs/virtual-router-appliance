...
VRRP virtual IP o VIP, è l'indirizzo IP mobile modificato dal dispositivo master al backup quando si verifica il failover. Quando una VRA esegue la distribuzione, avrà una connessione di rete associata privata e pubblica e IP reali assegnati ad ogni interfaccia. Un VIP viene assegnato ad entrambe le interfacce, indipendentemente dal fatto che il dispositivo sia autonomo o in una coppia HA. Il traffico con un IP di destinazione nelle sottoreti nelle VLAN associate alla VRA sarà inviato direttamente a questi VIP VRRP.
...
**NOTA:** nelle distribuzioni fornite da IBM© Cloud, il valore di Start Delay (Ritardo dell'avvio) è impostato sul valore predefinito. Potresti volerlo modificare a tua discrezione mentre verifichi i tuoi metodi di failover e alta disponibilità.
