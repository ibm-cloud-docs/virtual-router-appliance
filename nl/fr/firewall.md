...
## Sans et avec état
Par défaut, le pare-feu est configuré sans état, mais il peut être configuré avec état si nécessaire. Un pare-feu sans état nécessitera des règles pour le trafic bidirectionnel, alors que les pare-feux avec état seront adaptés aux connexions où seules les règles de trafic entrant sont requises. Pour configurer un pare-feu avec état, vous devez indiquer les connexions associées ou établies pour le trafic sortant.

Pour faire en sorte que tous les pare-feux soient "avec état", exécutez les commandes suivantes :
...
Pour que certains pare-feux soient "avec état" :
...
Une interface peut avoir plusieurs jeux de règles appliqués dans chaque direction. Ils sont appliqués dans l'ordre dans lequel ils ont été configurés. Notez qu'il n'est pas possible de créer un pare-feu pour le trafic en provenance de l'unité VRA en utilisant des pare-feux par interface.
...
Control Plane Policing (CPP) fournit une protection contre les attaques sur l'unité Virtual Router Appliance (VRA) en vous autorisant à configurer des règles d'administration de pare-feu affectées aux interfaces désirées et d'appliquer ces règles aux paquets entrant et sortant de l'unité VRA.

CPP est implémenté lorsque le mot-clé `local` est utilisé dans les règles d'administration de pare-feu affectées à n'importe quel type d'interface VRA, que ce soit des interfaces de plan de données ou de bouclage. Contrairement aux règles de pare-feu appliquées aux paquets qui transitent via l'unité VRA, l'action par défaut des règles de pare-feu pour le trafic entrant ou sortant du plan de données est `Allow`.  Les utilisateurs doivent ajouter des règles de suppression explicites si le comportement par défaut n'est pas souhaité.

L'unité VRA fournit une règle CPP de base comme modèle. Vous pouvez la fusionner dans votre configuration en exécutant cette commande :  

`vyatta@vrouter# merge /opt/vyatta/etc/cpp.conf`

Une fois ce jeu de règles fusionné, un nouveau jeu de règles nommé `CPP` est ajouté et appliqué à l'interface de bouclage. Il est recommandé de modifier ce jeu de règles pour l'adapter à votre environnement.
...
Les interfaces au sein de chaque zone peuvent autoriser le trafic librement et des règles peuvent être définies pour les interactions entre les zones. Un jeu de règles est configuré pour sortir d'une zone et entrer dans une autre. La commande suivante montre un exemple de configuration de règle :
...
*	Journalisation par paquet. Insérez le mot-clé ``log`` dans la règle NAT ou la règle de pare-feu pour consigner tous les paquets réseau correspondant à la règle.
