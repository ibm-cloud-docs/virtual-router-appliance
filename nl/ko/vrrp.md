...
VRA({{site.data.keyword.vra_full}})는 VRRP(Virtual Router Redundancy Protocol)를 고가용성 프로토콜로 지원합니다. 디바이스 배치는 활성/수동 방식으로 수행되며 여기서 한 시스템은 마스터이고 다른 시스템은 백업이 됩니다. 두 시스템의 모든 인터페이스는 동일한 "sync-group"의 멤버이므로 한 인터페이스에 결함이 있는 경우 동일한 그룹의 다른 인터페이스에도 결함이 발생하며 디바이스가 마스터가 되지 않습니다. 현재 백업은 마스터가 더 이상 킵얼라이브/하트비트 메시지를 브로드캐스트하지 않음을 발견하고 VRRP 가상 IP의 제어를 가정하여 마스터가 됩니다.
...
`dp0bond1` 또는 `dp0bond0`에 대한 VRRP 가상 IP 또는 VIP는 장애 복구가 발생할 때 마스터에서 백업 디바이스로 변경되는 유동 IP 주소입니다. VRA를 배치하면 공인 및 사설 본딩 네트워크 연결과 각 인터페이스에 지정된 실제 IP를 가지게 됩니다. VIP는 디바이스가 독립형인지 또는 HA 쌍인지 여부에 상관 없이 두 인터페이스 모두에 지정됩니다. VRA와 연관된 VLAN의 서브넷에 대상 IP가 있는 트래픽은 FCR/BCR의 정적 라우트를 통해 이 VRRP VIP에 직접 전송됩니다. 

게이트웨이 그룹에 대한 VRRP 가상 IP 주소를 변경해도 안 되고 VRRP 인터페이스를 사용 안함으로 설정해도 안 됩니다. 이러한 IP 주소는 VLAN이 연결될 때 트래픽이 게이트웨이로 라우트되는 수단입니다. 그 결과 이 IP 주소가 작동 중단되면 VLAN 트래픽도 작동 중단됩니다. IP 주소가 없는 경우에는 트래픽을 softLayer BCR/FCR에서 VRA 자체로 전달할 수 없습니다. 지금은 이 가상 주소 또는 VIP를 변경할 수 없습니다. 이러한 제한은 향후 변경될 수 있지만 현재는 POD/FCR/BCR 간 VRA 마이그레이션 및 VIP 변경이 지원되지 않습니다. 

특정 VRA에 대한 `dp0bond0` 및 `dp0bond1` VIP의 기본 구성 예는 다음과 같습니다. IP 주소 및 vrrp-group은 아래 예와 다를 수 있습니다. 
...
VIF에 대해 가상 주소를 구성하는 것에 대한 자세한 정보는 [단일 VLAN에 여러 서브넷 추가](/docs/infrastructure/virtual-router-appliance?topic=virtual-router-appliance-managing-your-vlans#add-multiple-subnets-to-a-single-vlan) 또는 [VRRP와 연관된 VLAN 서브넷](/docs/infrastructure/virtual-router-appliance?topic=virtual-router-appliance-working-with-high-availability-and-vrrp#associated-vlan-subnets-with-vrrp)을 참조하십시오. 

**기본적으로 VRRP는 사용 안함으로 설정됩니다. **이로 인해 새로 프로비저닝하거나 다시 로드해도 마스터 디바이스에서 가동 중단이 발생하지 않습니다. VLAN 트래픽을 작동시키기 위해 프로비저닝 또는 다시 로드가 완료되면 VRRP를 다시 사용으로 설정해야 합니다. 다음 예에서는 기본 구성에 대해 자세히 설명합니다. 
...
프로비저닝 또는 다시 로드 후 이 두 인터페이스에서 VRRP를 사용으로 설정하려면(**독립형 쌍과 HA 쌍 모두에 대해 필수임**) 다음 명령을 실행한 후 변경사항을 커미트하십시오. 
...
rfc-compatibility는 인터페이스에서 VRRP에 대한 RFC 3768 MAC 주소를 사용 또는 사용 안함으로 설정합니다. 이는 네이티브 인터페이스에서 사용되어야 하지만 구성된 VIF에서는 사용되지 않아야 합니다. 일부 가상 스위치(대부분 vmware)에서는 이를 사용하는 데 문제가 있으며 이로 인해 트래픽을 삭제하고 하이퍼바이저 호스트 시스템에서 게이트웨이 IP로 전송하지 않습니다. 이 설정만 그대로 두고 모든 VIF의 설정을 구성하지 마십시오.
...
VRA는 VRRP와 함께 한 쌍의 {{site.data.keyword.vra_full}}를 사용하여 하나의 IPsec 터널을 통해 연결을 유지보수하는 기능을 제공합니다. 한 라우터가 실패하거나 유지보수를 위해 작동이 중지되는 경우 새 VRRP 마스터 라우터가 로컬과 원격 네트워크 간의 IPsec 연결을 복원합니다.
...
VRRP를 사용한 VIF 구성에는 가상 IP 주소(서브넷에서 첫 번째로 사용 가능한 IP, 해당 서브넷의 모든 항목이 라우트되는 게이트웨이 IP) 및 두 디바이스의 VIF에 대한 실제 인터페이스 IP 주소가 필요합니다. 사용 가능한 IP를 유지하려면 실제 인터페이스 IP가 192.168.0.0/30과 같은 대역 외 범위를 사용하는 것이 좋습니다. 여기서 한 디바이스는 .1의 주소를 가지며 다른 디바이스는 .2의 주소를 가지게 됩니다. 여러 VRRP 가상 IP를 가질 수 있지만 각 VIF에 하나의 실제 주소만 필요합니다. 

두 개의 사설 VLAN과 세 개의 서브넷을 가진 VRRP 구성의 예는 다음과 같습니다. 
...
*  VLAN 인터페이스(VIF)의 vrrp-group 수는 일치하는 네이티브 인터페이스와 거의 항상 동일해야 합니다. 예를 들어, 두 인터페이스가 동일한 sync-group을 공유하는 경우가 아니면 `dp0bond1.789`는 항상 `dp0bond1`과 동일한 vrrp-group 수를 가지고 있어야 합니다. VIF에서 그룹 수가 다른 경우에는 다른 동기화 그룹과 쌍으로 지정되면 네이티브 인터페이스에 "분할뇌(split brain)" 조건이 발생할 수 있습니다. 두 인터페이스가 동일한 sync-group을 공유하는 경우에는 서로 다른 vrrp-group에 있더라도 두 인터페이스는 동시에 마스터와 백업 사이에서 상태 전이하여 분할뇌를 방지합니다. 반면에 네이티브 인터페이스가 VLAN 인터페이스와 다른 vrrp-group 수 및 다른 sync-group으로 설정된 경우 VLAN 인터페이스에서 설정된 서브넷이 네이티브 인터페이스의 가상 주소에 정적으로 라우팅되기 때문에 VLAN 인터페이스가 백업으로 표시되고 네이티브 인터페이스가 마스터인 경우에는 VLAN 인터페이스가 보조이고 VRA에서 활성 상태가 아닌 경우에도 라우터는 VLAN 인터페이스 트래픽을 네이티브 인터페이스가 마스터인 VRA에 계속 전송합니다. 이 특정 상황으로 인해 "분할뇌" 및 가동 중단이 발생합니다. sync-group이 vrrp-group과 함께 설정되는 방식에 유의해야 하는 것이 매우 중요한 이유가 이것입니다. 4개의 Vyatta가 동일한 그룹을 사용하면 3개의 VRA가 잠재적으로 백업 상태를 가정하고 하나만 마스터가 되어 가동 중단이 발생하므로 동일한 전송 VLAN의 여러 HA VRA 사이에 동일한 vrrp-group 수를 사용하지 않는 것도 매우 중요합니다. 
...
VRRP 장애 복구를 강제 실행해야 하는 경우에는 현재 VRRP 마스터인 디바이스에서 다음을 실행하여 이를 수행할 수 있습니다. 

`vyatta@vrouter$ reset VRRP master interface dp0bond0 group 1`

그룹 ID는 네이티브 인터페이스의 VRRP 그룹 ID이며 위에 언급한 대로 사용자의 쌍에서는 다를 수 있습니다. 

## 연결 동기화
{: #connection-synchronization}

두 개의 VRA 디바이스가 HA 쌍인 경우 두 디바이스 간의 Stateful 연결을 추적하는 것이 유용할 수 있습니다. 이렇게 하면 장애 복구가 발생하는 경우 실패한 디바이스에 있는 모든 연결의 현재 상태가 백업 디바이스로 복제되므로 현재 활성 세션(예: SSL 연결)을 처음부터 다시 빌드할 필요가 없습니다. 처음부터 다시 빌드하는 경우 사용자에게 혼란을 줄 수 있습니다.
...
IBM© Cloud에서 프로비저닝한 배치에서는 시작 지연 값이 기본값으로 설정됩니다. 장애 복구 및 고가용성 메소드를 테스트할 때 사용자 재량으로 이 값을 변경하려고 할 수 있습니다.
{: note}
