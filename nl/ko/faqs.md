...
## 두 Vyatta 디바이스를 고가용성(HA) 쌍으로 실행하려는 이유는 무엇입니까? 
{: faq}

대부분의 클라우드 고객은 고가용성(HA) 서비스를 원합니다. 왜냐하면 워크로드가 둘 이상의 완전히 독립된 (하드웨어) 시스템 또는 두 개의 독립된 가용성 구역(예: 데이터 센터)에서 호스팅되어 하나가 실패하면 다른 하나가 서비스를 계속할 수 있기 때문입니다. 한 시스템이 실패하면 다른 시스템으로 "장애 복구"가 수행되어 서비스를 계속 실행할 수 있습니다. 이것을 "HA" 서비스라고 하며 거의 항상 사용 가능합니다. 
