...
이 명령을 완료한 후 브라우저의 주소 표시줄에 `https://<ip.address>`를 입력하십시오. 여기서 IP 주소를 VRA의 IP 주소로 바꾸십시오. VRA의 자체 발행 인증서를 승인하도록 요청받을 수 있습니다. 이를 수행한 다음 프롬프트가 표시되면 Vyatta 신임 정보로 웹 인터페이스에 로그인하십시오. 
