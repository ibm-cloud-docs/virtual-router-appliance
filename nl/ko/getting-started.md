...
IBM© Virtual Router Appliance(VRA)는 x86 베어메탈 서버용 최신 Vyatta 5600 운영 체제를 제공합니다. VRA는 고가용성(HA) 또는 독립형 구성으로 제공되며, 이를 통해 방화벽, 트래픽 쉐이핑, 정책 기반 라우팅, VPN 및 기타 기능이 포함된 모든 기능을 갖춘 엔터프라이즈 라우터를 통해 사설 및 공용 네트워크 트래픽을 선택적으로 라우팅할 수 있습니다. 
...
## VRA(Virtual Router Appliance) 주문
{: #order-vra}
...
주문이 승인되면 VRA(Virtual Router Appliance) 프로비저닝이 자동으로 시작됩니다. 프로비저닝 프로세스가 완료되면 새 VRA가 게이트웨이 어플라이언스 목록 페이지에 표시됩니다. 게이트웨이 이름을 클릭하여 게이트웨이 세부사항 페이지를 여십시오. 디바이스의 IP 주소, 로그인 사용자 이름 및 비밀번호를 찾을 수 있습니다.  
...
연관된 VLAN의 서버는 VRA(Virtual Router Appliance)를 통해서만 다른 VLAN에서 연결될 수 있습니다. VLAN을 무시하거나 연관 해제하는 경우가 아니면 VRA를 우회할 수 없습니다. 
