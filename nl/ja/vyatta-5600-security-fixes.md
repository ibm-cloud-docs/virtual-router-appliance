...
| --- | --- | --- | --- |
| VRVDR-46824 | N/A | DSA-4440-1 | CVE-2018-5743、CVE-2018-5745、CVE-2019-6465: Debian DSA-4440-1 : bind9 - セキュリティー・アップデート |
| VRVDR-46603 | 5.3 | DSA-4435-1 | CVE-2019-7317: Debian DSA-4435-1 : libpng1.6 - セキュリティー・アップデート |
| VRVDR-46425 | N/A | DSA-4433-1 | CVE-2019-8320、CVE-2019-8321、CVE-2019-8322、CVE-2019-8323、CVE-2019-8324、CVE-2019-8325: Debian DSA-4433-1 : ruby2.3 - セキュリティー・アップデート |
...
| 発行番号 | 優先度 | 概要 |
| --- | --- | --- |
| VRVDR-46029 | 大 | 単純なテキスト・パスワードまたは AH タイプを使用した場合の VRRP 認証が正しく機能しない |
| VRVDR-45864 | 重大 | vyatta-techsupport リモート・コピーでシェル・インジェクションの特権エスカレーション/サンドボックス・エスケープが発生する |
| VRVDR-45748 | 大 | NULL ポインターを返す zmsg_popstr のチェックが行われないために、connsync によりデータプレーンのクラッシュが発生する  |
| VRVDR-45740 | 軽微 | 「generate tech-support archive」で既存のすべてのアーカイブを集約しない |
| VRVDR-45720 | 大 | start_delay が単一ルーターでのみ使用された場合に vrrp がパケット待機中に停止する |
| VRVDR-45655 | 重大 | VRRP フェイルオーバーの実行時に「PANIC in rte_mbuf_raw_alloc」が発生する |
| VRVDR-45059 | 大 | sip_expire_session_request の deref がヌルである |
| VRVDR-41419 | 大 | 静的分析データ・プレーンの修正 |
...
| VRVDR-46087 | N/A | DSA-4425-1 | CVE-2019-5953: Debian DSA-4425-1 : wget - セキュリティー・アップデート |
...
以下のコマンドはこのパッチ以降は非推奨となり、使用できなくなりました。
...
上記のいずれかのコマンドを実行すると、「この機能は無効です」というエラー・メッセージが表示されます。 
...
| VRVDR-45466 | 軽微 | 構成が PXE ブートによってロードされるときに IPv6 アドレスが簡略化されず、config-sync の問題が発生する |
| VRVDR-45414 | 軽微 | Vyatta-cpu-shield が開始に失敗し、2 ソケット・システム上のさまざまなコアに対して `OSError: [Errno 22] Invalid argument` がスローされる |
