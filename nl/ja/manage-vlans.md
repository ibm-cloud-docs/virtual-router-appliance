...
`virtual-address` は、新しいサブネットを構成する場所です。 ほとんどの場合、サブネットのゲートウェイ IP アドレスを構成する必要があります。 その後、VIF にバインドされたゲートウェイ IP は、VRA の背後にある新しいサブネットにセットアップされたベアメタル・サーバーまたは仮想サーバーのための、次のホップまたはデフォルト・ルートとして使用されます。
